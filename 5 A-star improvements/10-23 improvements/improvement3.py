...
# changes from 10/21:
# FOUND THE ERROR: When updating manhattan distances, only look at the space,
# not the other character being swapped. Fixed -- correct updateDict.

# attempt to fix the last, spectacular failure. If done right it should work.
# trying to figure out why it went significantly slower.
# also organization, because this is getting confusing and hard to debug.
# organization:
# - find/replace global variables to all caps, even though I don't like them, because it's true
# that they're easier to remember/read like that (sigh things look really aggressive now)
# - order things more logically
# - not letting lines go off the side of the screen
# - explain to myself what I'm doing w/ better comments

# comments from original bad version (improvement2):
# trying mnhtUpdate
# thought: not all keys in UPDATEMNHT are actually possible, and I could probably
# get only those pairs by going through the LOOKUPNBRS table. Want to fix in the
# future, just don't feel like it right now because that's not the focus.
# the same applied to MNHTCOORDINATES -- currently, they've both got 256 keys when they really
...
print(MNHTCOORDINATES[(5, GOALSPACE)], MNHTCOORDINATES[(4, GOALINDEXES['d'])], GOALINDEXES['d'])
print(MNHTCOORDINATES[(4, GOALSPACE)], MNHTCOORDINATES[(5, GOALINDEXES['d'])], GOALINDEXES['d'])
'''
...
def solve(puzzle, GOAL = '_abcdefghijklmno'):
    ...
    #print('Puzzle: {}, startMNHT: {}'.format(puzzle, currentMnht))
    ...
    while True:
        ...
        for nbr in neighbors(pzStr, space):
            ...
            if nbrPzl == GOAL:
                ...
                #print(nbrPzl, currentMnht, space, nbrSpace, newMnht)
...
for pzl in FILE:
    if lvl < 51:
        ...
        lvl = len(printList) - 1
        print(pzl.strip(), ': ', lvl, 'TIME:', elapsed, 'sec SEQUENCE:', printList) \
              if printList else (pzl, 'is not solvable.')
    if lvl == 51:
