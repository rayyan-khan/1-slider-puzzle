...
class Puzzle():
    ...
    def __init__(self, puzzle, level, wtr, pwtr):
        self.puzzle = puzzle
        self.level = level
        self.waysToReach = wtr
        self.prevwtr = pwtr
    ...
    def getWaysToReach(self):
        return self.waysToReach
    ...
    while parseMe:
        ...
        for k in neighbors(puzz.getPuzzle()):
            if k not in dictPuzzles:
                newPuzz = Puzzle(k, newLevel, puzz.getWaysToReach(), 0)
                ...
                try: #receiving a KeyError:4132_8765 -- confusing because the key is supposed to be the newLevel
                    if newLevel in dictPuzzles[k]: #try-except to work around this while I try to understand why
                        ...
                        parseMe.append(Puzzle(k, newLevel, puzz.getWaysToReach()+1, puzz.getWaysToReach()))
                except:
                    pass
        waysToReach += puzz.getWaysToReach()
        waysToReach -= puzz.getPrevWtr()
    return waysToReach
