...
# final version of 10/23 (cleaned up version of improvement3):
# changes from 10/21:

# 1: goal lookup table {letter: index}
# change grid from dict to list
# use it to get rid of .find in mnhtPuzzle, also using enumerate
# 2: mnhtUpdate -- dict = {(from, to, chr): +- 1}
# rather than recalculate the entire manhattan distance, look up the change in the dict
# 3: organization (because it got confusing and hard to debug):
...
# afterthought: not all keys in UPDATEMNHT are actually possible, and I could probably
