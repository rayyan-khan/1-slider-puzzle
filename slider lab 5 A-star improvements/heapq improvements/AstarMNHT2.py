...
#AstarMNHT1 improvements, using comprehensions rather than written out for-loops
# future: mnhtPuzzle only of necessary indeces? and cut down on find(k) -- tried but got errors
...
def mnhtPuzzle(puzzle, goal = '_abcdefghijklmno'): #see if work then comprehension
    ...
    for k in puzzle:
        if k != '_':
        pos = (puzzle.find(k), goal.find(k)) #from, to
        mnht += mnhtDict[pos]
    return mnht

def mnhtPuzzle1(puzzle, goal = '_abcdefghijklmno'):
    return sum([mnhtDict[puzzle.find(k), goal.find(k)] for k in puzzle])
