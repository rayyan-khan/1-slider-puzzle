...
# add updateEst dict (what dr gabor called FINCR)
# could probably reduce UPDATEDICT by a couple thousand if take mnhtDict off the last ones
        ...
        for to_ in LOOKUPNBRS[from_]:
            for ch in GOAL:
                if ch != '_':
                    if MNHTCOORDINATES[(to_, GOALINDEXES[ch])] \
                            > MNHTCOORDINATES[(from_, GOALINDEXES[ch])]:
                        UPDATE_EST[(from_, to_, ch, est)] = est
                    else:
                        UPDATE_EST[(from_, to_, ch, est)] = est + 2
    ...
    while True:
        ...
        for nbr in neighbors(pzStr, space):
            ...
            if nbrPzl == GOAL:
                ...
                newEst = UPDATE_EST[(space, nbrSpace, pzStr[nbrSpace], est)]
...
for pzl in FILE:
    if lvl < maxLevel:
        ...
        if lvl % 10 == 0 or lvl == maxLevel:
            print('Time: ', round(time.clock() - countTotalTime, 3))